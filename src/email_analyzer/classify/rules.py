"""Rule-based email classification (YAML first, then mailing-list heuristics)."""
...
CATEGORIES: tuple[Category, ...] = ("newsletter", "community", "other")

# Tokens matched against List-Id when YAML community rules miss.
_COMMUNITY_LIST_TOKENS = ("boost",)
...
def _list_id_text(msg: EmailMessage) -> str:
    return (msg.list_id or "").lower()


def _classify_by_heuristics(config: AppConfig, msg: EmailMessage) -> Category | None:
    labels = {label.upper() for label in msg.labels}
    list_id = _list_id_text(msg)

    if "CATEGORY_FORUMS" in labels:
        return "community"
    if list_id:
        for domain in config.sender_rules.community_domains:
            if domain.lower() in list_id:
                return "community"
        if any(token in list_id for token in _COMMUNITY_LIST_TOKENS):
            return "community"

    if list_id or (msg.list_unsubscribe or "").strip():
        return "newsletter"
    if (msg.precedence or "").strip().lower() == "list":
        return "newsletter"
    return None
...
def classify_message(config: AppConfig, msg: EmailMessage) -> Category:
    ...
    return classify_by_rules(config, msg) or _classify_by_heuristics(config, msg) or "other"


def group_messages_by_category(messages: list[EmailMessage]) -> dict[str, list[EmailMessage]]:
    by_cat: dict[str, list[EmailMessage]] = {cat: [] for cat in CATEGORIES}
    for msg in messages:
